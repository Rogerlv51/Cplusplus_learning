...
条件变量需要和互斥锁配合起来使用，它能阻止一个或多个线程，C++11提供了两种条件变量：
- `std::condition_variable`：需要配合`std::unique_lock<std::mutex>`进行wait操作，也就是阻塞线程的操作。
- `std::condition_variable_any`：可以和任意带有lock()、unlock()语义的mutex搭配使用（即之前提到的四种锁）

条件变量通常用于<mark>生产者和消费者模型，大致使用过程如下</mark>：
- 1、拥有条件变量的线程获取互斥量
...
`condition_variable`的成员函数主要分为两部分：<mark>线程等待（阻塞）函数 和线程通知（唤醒）函数</mark>，这些函数被定义于头文件 `<condition_variable>`
- 等待函数：调用`wait()`函数的线程会被阻塞
  ```c++
  // ①
